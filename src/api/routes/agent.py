...
@observe(name="agent_file_upload")
async def upload_file(
    ...
    try:
        ...
        with propagate_session_id(session_id, user_id=user_id, file_name=file.filename[:100]):
            ...
            if langfuse:
                try:
                    langfuse.update_current_trace(
                        ...
                        tags=["agent", "file_upload", "api"],
                        metadata={"file_name": file.filename[:100]},
                    )
            ...
            if background:
                try:
                    ...
                    job = await redis_pool.enqueue_job(
                        ...
                        session_id,
                        user_id,
                        _job_id=f"{user_id}:{job_priority}-{tracking_id}"
                    )
            ...
            # ✅ Synchronous processing (already inside propagate_session_id)
            try:
