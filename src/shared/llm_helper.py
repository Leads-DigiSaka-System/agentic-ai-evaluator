...
from src.core.config import GOOGLE_API_KEY, GEMINI_MODEL, LANGFUSE_CONFIGURED, GEMINI_LARGE
...
async def ainvoke_llm(prompt: str, as_json: bool = False, trace_name: str = None, model: str = GEMINI_MODEL):
    """
    Async version: Send a prompt to Gemini and return the response with Langfuse tracking (v3).
    
    ✅ MULTI-USER READY: Uses async ainvoke() which is non-blocking and allows concurrent requests.
    This is the recommended method for production with multiple users.
    ...
    try:
        ...
        # Create LLM instance with specified model (each call gets its own instance - thread-safe)
        callbacks = []
        handler = get_langfuse_handler()
        if handler:
            callbacks.append(handler)
        ...
        # ✅ Use async ainvoke() - non-blocking, allows concurrent requests
...
def invoke_llm(prompt: str, as_json: bool = False, trace_name: str = None, model: str = GEMINI_MODEL):
    """
    Synchronous version: Send a prompt to Gemini and return the response with Langfuse tracking (v3).
    
    ⚠️ NOTE: This uses blocking invoke() which can block the event loop in async contexts.
    For multi-user production, prefer ainvoke_llm() in async contexts.
    
    This function is kept for backward compatibility with synchronous workflow nodes.
    ...
    try:
        ...
        # Invoke LLM with the specified model (blocking call)
